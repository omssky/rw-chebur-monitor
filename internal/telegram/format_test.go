...
func TestCardCoverageGroupingAndMissingProbes(t *testing.T) {
	...
	require.Contains(t, text, "🟠 Частичная ТСПУ-блокировка")
	require.Contains(t, text, "Ответили <b>4 из 5</b>")
	require.Contains(t, text, "ТСПУ: <b>2</b>")
	require.Contains(t, text, "OK: <b>1</b>")
	require.Contains(t, text, "Неопределённо: <b>2</b>")
	...
	require.Contains(t, text, "Нет свежих данных: 1")
	require.Contains(t, text, "25.09.2026 13:00 МСК")
...
func TestClosedCardKeepsHistoryAndStrikesOnlyInlineContent(t *testing.T) {
	...
	text := render(monitor.Event{Kind: monitor.EventCard, Card: card})
	require.Contains(t, text, "<h3>🟢 Восстановлено · Poland</h3>")
...
func TestUnavailableAndStoppedNeverClaimRecovery(t *testing.T) {
	...
	text := render(monitor.Event{Kind: monitor.EventCard, Card: card})
	require.Contains(t, text, "Последняя успешная проверка: 25.09.2026 15:00 МСК")
	require.NotContains(t, text, "Проверено: 25.09.2026 16:00")
...
func TestSummaryAndEscalation(t *testing.T) {
	...
	require.Contains(t, summary, "🟢 Восстановлено")
	require.Contains(t, summary, "Длительность: 2 ч 0 мин")
	require.NotContains(t, summary, "<s>")
	escalation := render(monitor.Event{Kind: monitor.EventEscalation, Card: card})
	require.Contains(t, escalation, "Блокировка распространилась на все проверенные сети")
}
